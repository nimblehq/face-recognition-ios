...
final class FaceTrackingController: CameraViewController {

    let predictLabel = UILabel()

    private var detectFaceRequests: [VNDetectFaceRectanglesRequest]?
    private var sequenceRequestHandler = VNSequenceRequestHandler()
    private var cacheRequests: [VNTrackObjectRequest] = []
    private var faceClassificationRequest: VNCoreMLRequest {
        let request = ModelManager.shared.request { result in
    ...
    private func prepareVisionRequest() {
        let detectFaceRequest: VNDetectFaceRectanglesRequest = VNDetectFaceRectanglesRequest { (request, error) in
            ...
            guard let request = request as? VNDetectFaceRectanglesRequest,
                let results = request.results as? [VNFaceObservation], !results.isEmpty else {
                    return
            }
...
extension FaceTrackingController {
    private func drawFace(observation: VNFaceObservation) {
        ...
        if let image = currentImage?.oriented(exifOrientation).cropped(toFace: observation) {
            do {
                try VNImageRequestHandler(ciImage: image, options: [:]).perform([faceClassificationRequest])
            } catch {
    ...
    private func setupVisionDrawingLayers() {
        ...
        guard let rootLayer = self.rootLayer else {
            fatalError("View was not property initialized")
        }
